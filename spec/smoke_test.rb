module Smoke_test
shared_examples_for "API tests" do |tests| 
	tests.each do |key,api|
		title = api[:title] || "title missing"
		describe "#{key}: #{title}, {path:#{api[:path]}, userlevel: #{RSpec.configuration.userlevel}}" do
					before :all do
						if api[:type] == "post"
					...
					if (api[:restrictions] || -1) > RSpec.configuration.userlevel #-1 is to handel the case when authorization is universal.
						it "Is the proper status code returned for the unauthorized request: 403" do
				   		...
				   		it "Is the expected response returned" do
					   		...
					   		if api[:error_code] == "forbidden"
					   			expected = OUApi.error[:forbidden]
					   		else
